...
class TULA_level2 : AppCompatActivity() {
    ...
    private val totalTime = 10 * 60 * 1000L // 10 minutes
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ⭐ Logic copied from SANAYSAY_level2: Use a single, local SharedPreferences file.
        val prefs = getSharedPreferences("TULA_level2_Progress", MODE_PRIVATE)
        val highestScore = prefs.getInt("highest_score", 0)
        quizAttempts = prefs.getInt("quiz_attempts", 0)

        // Show last score and attempts if the user has played before
        ...
        // Show start button only if attempts are remaining
    ...
    private fun startQuiz() {
        binding.timerText.text = "10:00" // Set initial timer display
        quizStarted = true
    ...
    private fun finishQuiz() {
        ...
        // Also save to the central "UserScores" file for MarkaPage
        val userScoresPrefs = getSharedPreferences("UserScores", Context.MODE_PRIVATE)
        userScoresPrefs.edit().putInt("TULA_LEVEL_2", newHigh).apply()
        ...
        // ⭐ Logic copied from SANAYSAY_level2: Set the listener here.
        binding.finishQuizBtn.setOnClickListener {
            // Save progress to the local file
            prefs.edit().apply {
            ...
            // Set result to unlock the next level
    ...
    // --- NO CHANGES NEEDED BELOW THIS LINE (Helper Functions) ---
    ...
    private fun shuffleQuestionsAndChoices() {
        ...
        tempQuestions.forEach { q ->
            val originalCorrectAnswer = q.choices[q.correctIndex]
            q.choices.shuffle()
            q.correctIndex = q.choices.indexOf(originalCorrectAnswer)
        }
